...
def task(args):
    ...
    bin = bin.joinpath("cosa_helper")

    bin_module = temp_dir.joinpath("cosa_helper")
    shutil.copy2(bin, bin_module)
    tools.strip(bin_module)

    build_time = datetime.now().strftime("%Y-%m-%d-%Hh%Mm%Ss")
    build_type = "release" if release else "debug"
    output = Path("output") / f"cosa_helper_{build_type}_{build_time}"
    ...
    print("cosa_helper build successfully: {}.zip".format(output))
